...
example1c_path = os.path.join(this_path, "../../examples/test/example1c.nc")

def example1c_save():
    print("Example 1c save")

    # check the target directory exists
    dir = os.path.dirname(example1c_path)
    if not (os.path.exists(dir)):
        os.mkdir(dir)

    # create the Dataset (AggregationContainer)
    ds = CFADataset(example1c_path, mode="w", format=CFAFileFormat.CFANetCDF)
    ...
    # for this example we need to serialise early so that we can add an attribute to
    # the "aggregation_file" variable
    ds.CFA.serialise()
...
def example1c_load():
    print("Example 1c load")
    # load in the CFA Dataset
    ds = CFADataset(example1c_path, mode="r", format=CFAFileFormat.CFANetCDF)
